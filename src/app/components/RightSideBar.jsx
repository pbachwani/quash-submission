...
const RightSideBar = ({ onGenerate, isGenerating }) => {
  ...
  return (
    <aside className="w-80 bg-[#1C1F22] p-4 flex flex-col gap-6">
      <div>
      ...
      {isGenerating && (
        <div className="mt-4 p-3 bg-[#333124] rounded">
          <div className="flex items-center space-x-2">
            <div className="w-2 h-2 bg-yellow-400 rounded-full animate-pulse" />
            <span className="text-xs text-[#FFD233]">
              Generating test cases
            </span>
            <div className="w-3 h-3 border border-[#FFD233] border-t-transparent rounded-full animate-spin" />
          </div>
        </div>
      )}
    </aside>
